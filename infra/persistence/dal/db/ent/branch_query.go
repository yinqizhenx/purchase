...
func (bq *BranchQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (bq *BranchQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(branch.Table, branch.Columns, sqlgraph.NewFieldSpec(branch.FieldID, field.TypeInt))
	_spec.From = bq.sql
...
func (bq *BranchQuery) ForUpdate(opts ...sql.LockOption) *BranchQuery {
...
func (bq *BranchQuery) ForShare(opts ...sql.LockOption) *BranchQuery {
	if bq.driver.Dialect() == dialect.Postgres {
		bq.Unique(false)
	}
	bq.modifiers = append(bq.modifiers, func(s *sql.Selector) {
