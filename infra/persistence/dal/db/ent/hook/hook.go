...
type AsyncTaskFunc func(context.Context, *ent.AsyncTaskMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AsyncTaskFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AsyncTaskMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f PAHeadFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PAHeadMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PAHeadMutation", m)
}

// The PARowFunc type is an adapter to allow the use of ordinary
// function as PARow mutator.
type PARowFunc func(context.Context, *ent.PARowMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PARowFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PARowMutation); ok {
		return f(ctx, mv)
	}
